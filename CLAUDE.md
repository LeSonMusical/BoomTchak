...
| Version | Changements |
|---------|-------------|
| v3.28.43 | Fix double-tap bouton signature GCB : `setTimeout(50)` avant `_openSigAdvisorModal()` dans `pointerup` — sur iOS Safari, `rAF` peut s'exécuter avant le `click` natif → modal `.open` (pointer-events:all) est frappé par le click natif → backdrop ferme immédiatement ; le délai 50 ms garantit que le click natif se consume avant l'ouverture de la modal |
